...
export default function CalendarCell({isToday, date, roster, setRoster}: CalendarCellProps) {
    ...
    return (
        <>
            <div>
                <div
                    className={`h-36 border border-1 border-gray-200 print:border-black 
                    ...
                    <div className="px-2 pt-1 font-inter">
                        {rosterEntries.map(({tag, staff}) => {
                            // Find the matching field to get the color
                            const tagConfig = formConfig.fields.find(field => field.label === tag.label);
                            
                            return (
                                <div key={tag.label} className="flex flex-row items-center">
                                    <p 
                                        className={`font-inter font-semibold text-sm mb-1 mr-2 w-[30px] border-r-2 border-gray-200 print:border-black`}
                                        style={{ 
                                            color: isToday ? 'white' : tagConfig?.color,
